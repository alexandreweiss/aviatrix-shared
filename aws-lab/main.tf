...
resource "aws_subnet" "subnet" {
  ...
  vpc_id     = aws_vpc.vpc_r1.id
  cidr_block = each.key == "subnet-internal-a" ? cidrsubnet(var.vpc_cidr, 4, 0) : each.key == "subnet-internal-b" ? cidrsubnet(var.vpc_cidr, 4, 1) : each.key == "subnet-public-a" ? cidrsubnet(var.vpc_cidr, 4, 2) : each.key == "subnet-internal-bgp-lan-a" ? cidrsubnet(var.vpc_cidr, 4, 3) : each.key == "subnet-internal-bgp-lan-b" ? cidrsubnet(var.vpc_cidr, 4, 4) : cidrsubnet(var.vpc_cidr, 4, 5)
...
resource "aws_route_table_association" "rt-association" {
  for_each = { for k, v in aws_subnet.subnet : k => v if length(regexall("subnet-.*", k)) == 0 }
...
#   subnet_ids         = [aws_subnet.subnet["subnet-public-a"].id, aws_subnet.subnet["subnet-public-b"].id]
