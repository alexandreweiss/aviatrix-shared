...
## Creation of VPN GW
module "vpn_gw" {
  source = "github.com/alexandreweiss/misc-tf-modules.git/vpn-gateway"

  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  gateway_name        = "vpn-${var.azure_r1_location_short}-gw"
  gw_subnet_id        = azurerm_subnet.gw-subnet.id
}

#Vnet peering with Transit
## Grab transit data
...
## Create peering between VPN GW vnet and transit vnet
module "vn-peering" {
  source = "github.com/alexandreweiss/terraform-azurerm-vnetpeering"

  left_vnet_resource_group_name  = azurerm_resource_group.rg.name
  left_vnet_name                 = azurerm_virtual_network.vnet.name
  right_vnet_resource_group_name = "azr-transit-we-0-avx-rg"
  right_vnet_name                = data.tfe_outputs.dataplane.values.transit_we.vpc.name
...
resource "azurerm_local_network_gateway" "lng0" {
  ...
  name                = "ferme"
  resource_group_name = azurerm_resource_group.rg.name
  gateway_address     = data.dns_a_record_set.ferme.addrs[0]
  bgp_settings {
    asn                 = 65510
...
resource "azurerm_virtual_network_gateway_connection" "vpn-gw-conn-0" {
  ...
  name                       = "vpn-gw-conn-0"
  resource_group_name        = azurerm_resource_group.rg.name
  virtual_network_gateway_id = module.vpn_gw.vpn_gateway.id
